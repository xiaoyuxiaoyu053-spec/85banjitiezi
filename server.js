...
app.post("/api/profile", async (req, res) => {
  try {
    ...
    const name =
      clean(req.body.display_name, 30) ||
      "匿名用户";

    const avatar =
      clean(req.body.avatar, 500) ||
      "default";
  ...
  try {
    ...
    const type =
      [
        ...
        "comment",
        "message"
      ].includes(req.body.target_type)
        ? req.body.target_type
        : "post";

    const reason =
      clean(req.body.reason, 300);
...
app.post(
  ...
  async (req, res) => {
    ...
    const c =
      clean(req.body.content, 1000);

    if (c) {

      await pool.query(
        "INSERT INTO notices(content) VALUES($1)",
        [c]
      );
    }
